...
import { Search, MapPin, ShieldCheck, Users, BookOpen, ArrowRight, CheckCircle2 } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    api.get("/teachers?limit=6").then(res => setFeaturedTutors(res.data)).catch(() => {});
    ...
    <div className="min-h-screen bg-[color:var(--bg)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          <div className="lg:col-span-7 flex flex-col gap-6">
            ...
            <h1 className="font-display font-black text-4xl sm:text-5xl lg:text-6xl tracking-tight leading-[1.1] text-[color:var(--ink)]">
              Find verified home tutors in <span className="text-[color:var(--terracotta)]">Dibrugarh</span>
            </h1>
            <p className="text-base sm:text-lg text-[color:var(--ink-soft)] leading-relaxed max-w-2xl">
              Connect directly with qualified private tutors across Chowkidingee, Amolapatty, Naliapool, Milan Nagar, and all local areas for CBSE, SEBA, & College boards.
            </p>

            {/* Search Box */}
            <form onSubmit={handleSearch} className="bg-white p-2.5 sm:p-3 rounded-2xl sm:rounded-full border border-[color:var(--border-earth)] shadow-sm flex flex-col sm:flex-row items-center gap-2 mt-2">
              <div className="flex items-center gap-2 px-3 flex-1 w-full">
                <Search size={18} className="text-[color:var(--ink-soft)] shrink-0" />
                <input
              ...
              <div className="flex items-center gap-2 px-3 py-1 sm:border-l border-[color:var(--border-earth)] w-full sm:w-auto">
                <MapPin size={18} className="text-[color:var(--ink-soft)] shrink-0" />
                <select
            ...
            <div className="grid grid-cols-3 gap-4 pt-6 border-t border-[color:var(--border-earth)] mt-4">
              <div>
                <div className="font-display font-bold text-2xl text-[color:var(--ink)]">{stats?.verified_tutors ?? "6+"}</div>
                <div className="text-xs text-[color:var(--ink-soft)]">Verified Tutors</div>
              </div>
              <div>
                <div className="font-display font-bold text-2xl text-[color:var(--ink)]">{stats?.areas ?? "25+"}</div>
                <div className="text-xs text-[color:var(--ink-soft)]">Local Localities</div>
              </div>
              <div>
                <div className="font-display font-bold text-2xl text-[color:var(--ink)]">100%</div>
                <div className="text-xs text-[color:var(--ink-soft)]">Direct Connection</div>
              </div>
            </div>
          </div>
            ...
            <div className="relative rounded-3xl overflow-hidden border border-[color:var(--border-earth)] shadow-xl bg-[color:var(--surface)]">
              ...
              <div className="p-6 bg-white/90 backdrop-blur-md">
                <div className="flex items-center gap-2 text-sm font-semibold text-[color:var(--ink)]">
                  <ShieldCheck className="text-[color:var(--terracotta)]" size={18} />
                  Safe & Transparent Tutor Matching
                </div>
                <p className="text-xs text-[color:var(--ink-soft)] mt-1">
                  Contact details are securely revealed upon teacher confirmation to protect privacy and eliminate spam.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-10">
          <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-8">
            <div>
              ...
              <h2 className="font-display font-black text-2xl sm:text-3xl mt-1 text-[color:var(--ink)]">Top Rated Local Tutors</h2>
            </div>
            <Link to="/tutors" className="text-sm font-semibold text-[color:var(--terracotta)] hover:underline flex items-center gap-1">
      ...
      <section className="py-20 px-5 lg:px-10 max-w-7xl mx-auto">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <h2 className="font-display font-black text-3xl sm:text-4xl mt-1 text-[color:var(--ink)]">Built specifically for Dibrugarh</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-white p-8 rounded-2xl border border-[color:var(--border-earth)] flex flex-col gap-3">
            ...
            <h3 className="font-display font-bold text-lg">Hyper-Local Matching</h3>
            <p className="text-sm text-[color:var(--ink-soft)] leading-relaxed">
              Target tutors within walking or short commuting distance across all Dibrugarh pin codes and wards.
            </p>
          </div>
          ...
          <div className="bg-white p-8 rounded-2xl border border-[color:var(--border-earth)] flex flex-col gap-3">
            ...
            <h3 className="font-display font-bold text-lg">Admin Verified Credentials</h3>
            <p className="text-sm text-[color:var(--ink-soft)] leading-relaxed">
              Teachers' qualifications and experience are screened to ensure quality academic mentorship.
            </p>
          </div>
          ...
          <div className="bg-white p-8 rounded-2xl border border-[color:var(--border-earth)] flex flex-col gap-3">
            ...
            <h3 className="font-display font-bold text-lg">Zero Middleman Commission</h3>
            <p className="text-sm text-[color:var(--ink-soft)] leading-relaxed">
              Parents connect directly with educators. No hidden platform cuts or exorbitant broker charges.
            </p>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-[color:var(--border-earth)] bg-white py-12 px-5 lg:px-10 text-center text-xs text-[color:var(--ink-soft)]">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>© {new Date().getFullYear()} Tuitora Dibrugarh. All rights reserved.</div>
          <div className="flex gap-6">
            <Link to="/tutors" className="hover:underline">Find Tutors</Link>
            <Link to="/signup?role=teacher" className="hover:underline">Become a Tutor</Link>
