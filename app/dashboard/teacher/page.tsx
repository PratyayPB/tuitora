...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    fetch("/api/meta/areas")
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          setAreas(data);
          if (data.length > 0 && !locationArea) setLocationArea(data[0]);
        }
      })
      .catch(() => {});
    ...
    fetchProfile();
    fetchRequests();
  }, [fetchProfile, fetchRequests, locationArea]);
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        fee_per_month: Number(feePerMonth),
        fee_per_hour: feePerHour ? Number(feePerHour) : undefined,
  ...
  const handleToggleAvailability = async () => {
    try {
      ...
      if (res.ok) {
        ...
        toast.success(
          `Tuition status set to: ${
            newStatus ? "Accepting Students" : "Full / Unavailable"
          }`
        );
      }
    } catch {
  ...
  return (
    <div className="min-h-screen bg-[color:var(--bg)] py-10 px-5 lg:px-10 max-w-7xl mx-auto">
        ...
        {profile && (
          <div className="flex items-center gap-3">
            ...
            <Button asChild variant="outline" className="rounded-full">
              <Link href={`/teachers/${profile.id}`}>View Public Profile</Link>
            </Button>
          </div>
        )}
      </div>
      ...
      <div className="mb-8 p-4 rounded-2xl border flex items-center justify-between gap-4 bg-white border-[color:var(--border-earth)]">
        <div className="flex items-center gap-3">
          {profile?.is_verified ? (
            <div className="w-9 h-9 rounded-full bg-green-100 text-green-700 grid place-items-center shrink-0">
              <UserCheck size={20} />
            </div>
          ) : (
          ...
          {requests.length === 0 ? (
            <div className="bg-white rounded-2xl border border-[color:var(--border-earth)] p-12 text-center">
              ...
              <p className="text-xs text-[color:var(--ink-soft)]">
                Make sure your profile is complete with subjects and locality to
                  ...
                  <div>
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        {r.budget && (
                          <div className="text-xs font-semibold text-[color:var(--ink)] mt-1">
                            Proposed Budget: ₹{r.budget}/mo
                          </div>
                        )}
                      </div>
                      <span
                    ...
                    <div className="bg-green-50 border border-green-200 p-3 rounded-xl flex items-center justify-between text-xs text-green-900">
                      <div>
                        <span className="font-semibold block">
                          Student Contact Revealed:
                        </span>
                        <span className="flex items-center gap-1 mt-0.5">
                          <Phone size={12} /> {r.student_phone || "Phone not provided"}
                        </span>
                      </div>
                      {r.student_phone && (
                        <a
                          href={`tel:${r.student_phone}`}
        ...
        <div className="bg-white p-6 sm:p-10 rounded-3xl border border-[color:var(--border-earth)] shadow-sm max-w-3xl">
          <div className="mb-6 pb-4 border-b border-[color:var(--border-earth)]">
            ...
            <p className="text-xs text-[color:var(--ink-soft)] mt-0.5">
              Accurate details ensure you get inquiries matching your preferred
              classes and localities in Dibrugarh
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  min="0"
                  value={feePerMonth}
                  onChange={(e) => setFeePerMonth(Number(e.target.value))}
                  className="w-full text-xs border border-[color:var(--border-earth)] rounded-xl p-2.5 outline-none"
                />
              </div>
