...
function TutorSearchContent() {
      ...
      fetch("/api/saved")
        ...
        .then((data) => {
          if (Array.isArray(data)) {
            setSavedIds(new Set(data.map((t: any) => t.id)));
          }
        })
        .catch(() => {});
    }
  ...
  const handleToggleSave = async (tutor: TeacherProfile) => {
    if (!user) {
      toast.info("Please sign in to save tutors");
      return;
    }
    ...
    try {
      if (isSaved) {
        ...
        const res = await fetch(`/api/saved/${tutor.id}`, { method: "POST" });
        if (res.ok) {
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-[color:var(--border-earth)] p-12 text-center">
              ...
              <p className="text-xs text-[color:var(--ink-soft)] max-w-sm mx-auto mb-4">
                Try loosening your filters or resetting the search to see all
                registered tutors across Dibrugarh.
              </p>
              <Button
                onClick={handleReset}
                variant="outline"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {tutors.map((tutor) => (
                <TutorCard
                  key={tutor.id}
                  t={tutor}
                  onSave={user ? handleToggleSave : undefined}
