...
import { TeacherProfile } from "@/components/TutorCard";
import {
  BadgeCheck,
  MapPin,
  GraduationCap,
...
export default function TutorDetailPage() {
  ...
  const fetchTutor = useCallback(async () => {
    try {
      ...
      setTutor(data);
      if (data.subjects?.length > 0) setRequestSubject(data.subjects[0]);
      if (data.classes?.length > 0) setRequestClass(data.classes[0]);
      setRequestArea(data.location_area || "");
    } catch {
  ...
  const handleSendRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) {
      toast.info("Please sign in to contact this tutor");
      router.push("/sign-in");
      return;
    }
    ...
    try {
      ...
      toast.success(
        "Tuition request sent successfully! The teacher will review it."
      );
  ...
  const handleSendReport = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/reports", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          target_user_id: tutor?.user_id || tutor?.id,
          reason: reportReason,
        }),
      });
  ...
  if (loading) {
    return (
      <div className="min-h-screen py-20 text-center text-sm text-[color:var(--ink-soft)]">
  ...
  if (!tutor) {
    return (
      <div className="min-h-screen py-20 text-center">
        ...
        <Button asChild>
          <Link href="/teachers">Browse All Tutors</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[color:var(--bg)] py-10 px-5 lg:px-10 max-w-5xl mx-auto">
      <Link
        href="/teachers"
        className="inline-flex items-center gap-1.5 text-xs font-semibold text-[color:var(--ink-soft)] hover:text-[color:var(--terracotta)] mb-6"
      >
      ...
      <div className="bg-white rounded-3xl border border-[color:var(--border-earth)] p-6 sm:p-10 shadow-sm flex flex-col gap-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pb-6 border-b border-[color:var(--border-earth)]">
          <div className="flex items-center gap-5">
            <div className="w-20 h-20 rounded-full bg-[color:var(--surface)] border border-[color:var(--border-earth)] grid place-items-center font-display font-black text-2xl text-[color:var(--ink)] shrink-0">
              {initials}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1
                  className="font-display font-black text-2xl sm:text-3xl text-[color:var(--ink)]"
                  data-testid="tutor-detail-name"
                >
                  {tutor.name}
                </h1>
                {tutor.is_verified && (
              ...
              <div className="text-sm text-[color:var(--ink-soft)] flex flex-wrap items-center gap-3 mt-1">
                <span className="flex items-center gap-1">
                  <GraduationCap size={15} /> {tutor.qualification}
                </span>
                <span>•</span>
                <span>{tutor.experience_years} Years Experience</span>
                <span>•</span>
                <span className="flex items-center gap-1">
                  <MapPin size={15} /> {tutor.location_area}, Dibrugarh
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="text-xs text-[color:var(--ink-soft)] mt-1">
              {tutor.phone ? (
                ...
                <span className="text-neutral-500 italic">
                  Revealed after request is accepted
                </span>
              )}
            </div>
          </div>
        ...
        {tutor.description && (
          <div>
            <h2 className="font-display font-bold text-lg mb-2 text-[color:var(--ink)]">
              About the Tutor
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            <h2 className="font-display font-bold text-base mb-3 text-[color:var(--ink)]">
              Subjects Taught
            </h2>
            <div className="flex flex-wrap gap-2">
              {tutor.subjects?.map((s) => (
                <span key={s} className="tag-sage !text-xs !py-1 !px-3">
                  {s}
                </span>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {tutor.classes?.map((c) => (
                <span key={c} className="tag-terra !text-xs !py-1 !px-3">
                  {c}
                </span>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-[color:var(--border-earth)]">
          <div>
            ...
            <div className="text-sm text-[color:var(--ink-soft)] flex flex-col gap-1.5 mt-2">
              <div>
                <strong>Days:</strong>{" "}
                {tutor.availability_days?.length
                  ? tutor.availability_days.join(", ")
                  : "Flexible"}
              </div>
              <div>
                <strong>Time:</strong>{" "}
                {tutor.availability_time || "Flexible timings"}
              </div>
            </div>
          </div>

          <div>
            <h2 className="font-display font-bold text-base mb-2 text-[color:var(--ink)]">
              Tuition Modes Offered
            </h2>
            <div className="flex flex-wrap gap-2 mt-2">
              {tutor.tuition_modes?.map((m) => (
                <span
                  key={m}
                  className="px-3 py-1 rounded-full text-xs font-medium bg-neutral-100 border border-[color:var(--border-earth)] capitalize"
                >
                  {m.replace("_", " ")}
                </span>
              ))}
      ...
      {showRequestModal && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full border border-[color:var(--border-earth)] shadow-2xl">
            <h3 className="font-display font-bold text-xl text-[color:var(--ink)]">
              Request Tuition with {tutor.name}
            </h3>
            <p className="text-xs text-[color:var(--ink-soft)] mt-1 mb-5">
              Specify your subject and timing requirements. Once the tutor
              accepts, their direct contact details will be shared.
            </p>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full border border-[color:var(--border-earth)] shadow-2xl">
            ...
            <p className="text-xs text-[color:var(--ink-soft)] mt-1 mb-4">
              Please explain why you are reporting {tutor.name}s profile to
